...
--> statement-breakpoint
ALTER TABLE "answer_inference_leases" ADD CONSTRAINT "answer_inference_leases_workspace_id_workspace_inference_states_workspace_id_fk" FOREIGN KEY ("workspace_id") REFERENCES "public"."workspace_inference_states"("workspace_id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "workspace_inference_states" ADD CONSTRAINT "workspace_inference_states_workspace_id_workspaces_id_fk" FOREIGN KEY ("workspace_id") REFERENCES "public"."workspaces"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
CREATE UNIQUE INDEX "answer_inference_leases_id_workspace_unique" ON "answer_inference_leases" USING btree ("id","workspace_id");--> statement-breakpoint
CREATE INDEX "answer_inference_leases_workspace_status_expires_index" ON "answer_inference_leases" USING btree ("workspace_id","status","expires_at");--> statement-breakpoint
CREATE INDEX "answer_inference_leases_workspace_started_index" ON "answer_inference_leases" USING btree ("workspace_id","started_at");