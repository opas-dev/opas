// ABOUTME: Renders one sanitized published article from the request-time database.
// ABOUTME: Verifies both category and article slugs before exposing content to readers.
import Link from "next/link";
import { notFound } from "next/navigation";
import { connection } from "next/server";

import { PublicHeader } from "@/app/public-header";
import { RuntimeMdx } from "@/content/runtime-mdx";
import { getRepository } from "@/db";
import { demoIds } from "@/db/demo";

export const runtime = "nodejs";

const dateFormatter = new Intl.DateTimeFormat("en", {
  day: "numeric",
  month: "long",
  year: "numeric",
  timeZone: "UTC",
});

export default async function ArticlePage({
  params,
}: PageProps<"/[categorySlug]/[articleSlug]">) {
  const { categorySlug, articleSlug } = await params;
  await connection();
  const repository = await getRepository();
  const [categories, article] = await Promise.all([
    repository.listCategories(demoIds.workspace),
    repository.findPublishedArticle(demoIds.workspace, articleSlug),
  ]);
  const category = categories.find((candidate) => candidate.slug === categorySlug);

  if (!category || !article || article.categoryId !== category.id) {
    notFound();
  }

  return (
    <main>
      <PublicHeader />
      <div className="article-shell">
        <nav className="article-nav" aria-label="Breadcrumb">
          <Link href="/">OPAS</Link>
          <span aria-hidden="true">/</span>
          <Link href={`/${category.slug}`}>{category.name}</Link>
          <span aria-hidden="true">/</span>
          <span aria-current="page">{article.title}</span>
        </nav>
        <article className="article-content">
          <RuntimeMdx source={article.mdx} />
          <footer className="article-meta">
            <span>Written by {article.authorName}</span>
            <time dateTime={article.updatedAt.toISOString()}>
              Last updated {dateFormatter.format(article.updatedAt)}
            </time>
          </footer>
        </article>
      </div>
    </main>
  );
}
