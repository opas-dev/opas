// ABOUTME: Persists private article drafts and immutable revisions on Postgres and Neon.
// ABOUTME: Locks authorization, category, head, slug, and asset prerequisites before each write.
import {
  asc,
  and,
  desc,
...
function actorAssertion(
  ...
  const allowedRoles = sql.join(roles.map((role) => sql`${role}`), sql`, `);
  return sql`
    ...
    from (
      ...
      where member.workspace_id = ${workspaceId}
        ...
        and member.role in (${allowedRoles})
...
function contentReadAuthorizationCondition(
  ...
  return sql`exists (
    ...
    where authorized_member.workspace_id = ${workspaceId}
      ...
      and authorized_session.expires_at > ${authorization.checkedAt}
  )`;
}

function categoryAssertion(
  workspaceId: string,
  category: CategorySnapshot,
  exclusive = false,
) {
  return sql`
    ...
    from (
      ...
      ${exclusive ? sql`for update of category` : sql`for share of category`}
    ) current_category
  `;
}
...
function lockSlugStatements(workspaceId: string, articleId: string, slug: string) {
  return [
    sql`select id from articles where workspace_id = ${workspaceId} and (id = ${articleId} or slug = ${slug}) for update`,
    sql`select normalized_slug from article_slug_claims where workspace_id = ${workspaceId} and normalized_slug = ${slug} for update`,
  ];
}
...
function slugAssertion(
  ...
  return sql`
    select 1 / case when
      ...
      and (${creating} = false or not exists (
        select 1 from articles
...
function lockAssetStatements(
  ...
  if (hashes.length > 0) {
    statements.push(
      sql`select id from assets where workspace_id = ${workspaceId} and hash in (${sql.join(
  ...
  if (manifestId) {
    statements.push(
      sql`select id from asset_manifests where workspace_id = ${workspaceId} and id = ${manifestId} for share`,
    );
  }
  return statements;
}
...
function assetAssertion(
  ...
  return sql`
    with requested(hash) as (
      select distinct value from jsonb_array_elements_text(${serializedHashes}::jsonb)
    ), allowed(hash) as (
      select asset.hash
      from assets asset
    ...
    select 1 / case when
      (select count(*) from requested) = (select count(*) from allowed)
      and (${manifestId ?? null}::text is null or exists (
...
function upsertSlugClaim(
  ...
  return sql`
    ...
    ) values (${workspaceId}, ${slug}, ${articleId}, true, ${articleRowClaim})
    on conflict (workspace_id, normalized_slug) do update set
      working_claim = case
        when article_slug_claims.article_id = excluded.article_id then true
        else article_slug_claims.working_claim
      end,
      article_row_claim = case
        when article_slug_claims.article_id = excluded.article_id
          then article_slug_claims.article_row_claim or excluded.article_row_claim
...
function consumeManifestAndOrphans(
  ...
  return [
    ...
    sql`
      ...
      where asset.workspace_id = ${workspaceId}
        and not exists (select 1 from article_assets where asset_id = asset.id)
        and not exists (select 1 from article_revision_assets where asset_id = asset.id)
        and not exists (select 1 from asset_manifest_items where asset_id = asset.id)
    `,
  ];
}
...
function revisionInsert(
  ...
  return sql`
    insert into article_revisions (
      ...
      ${request.article.isFaq}, ${request.article.authorName}, ${request.article.position},
      ${revisionHash}, ${request.changeKind}, ${request.actor.memberId}, null,
      ${draftChangeSummary(request.changeSummary)}, ${changedAt}, null
    )
  `;
}
...
function rollbackRevisionInsert(
  ...
  return sql`
    insert into article_revisions (
      ...
      ${source.mdx}, ${source.isFaq}, ${source.authorName}, ${source.position},
      ${revisionHash}, 'rollback', ${request.actor.memberId}, null,
      ${draftChangeSummary(request.changeSummary)}, ${changedAt}, ${source.revisionId}
    )
  `;
}
...
async function actorIsAuthorized(
  ...
  const [authorizedActor] = await database
...
async function listAuthorizedRevisionHistory(
  ...
  const rows = await database
    .select({
      ...
      createdByMemberId: articleRevisions.createdByMemberId,
      isPublishedRevision: sql<boolean>`coalesce(${articleHeads.publishedRevisionId} = ${articleRevisions.id}, false)`,
      isWorkingRevision: sql<boolean>`${articleHeads.workingRevisionId} = ${articleRevisions.id}`,
      restoredFromRevisionId: articleRevisions.restoredFromRevisionId,
      revisionHash: articleRevisions.revisionHash,
      revisionId: articleRevisions.id,
      revisionNumber: articleRevisions.revisionNumber,
...
async function slugIsAvailable(
  ...
  const [foreignClaim, foreignArticle] = await Promise.all([
    database
  ...
  return (
    (!foreignClaim[0] || foreignClaim[0].articleId === articleId) &&
    (!foreignArticle[0] || foreignArticle[0].id === articleId)
  );
}
...
async function assetsAreAvailable(
  ...
  const matchingAssets = await database
...
function workflowHeadAssertion(
  ...
  const states = sql.join(allowedStates.map((reviewState) => sql`${reviewState}`), sql`, `);
  return sql`
    select 1 / count(*)::integer
    from (
      select head.article_id
      ...
      where head.workspace_id = ${request.workspaceId}
        ...
        and head.review_state in (${states})
        and head.archived_at is null
        and article.status = ${state.articleStatus}
        and (${requirePublished} = false or article.status = 'published')
      for update of head, article
    ) current_head
  `;
}
...
function submittingActorAssertion(request: WorkflowRequest) {
  return sql`
    select 1 / case when ${request.actor.memberId} = (
...
function revisionAssetSetAssertion(
  ...
  return sql`
    ...
    select 1 / case when
      (select count(*) from expected) = (select count(*) from stored)
      and not exists (select hash from expected except select hash from stored)
      and not exists (select hash from stored except select hash from expected)
      then 1 else 0 end
  `;
}
...
function restoredRevisionEventInsert(
  ...
  return sql`
    insert into article_review_events (
      ...
      ${eventId}, ${request.workspaceId}, ${request.articleId}, ${revisionId},
      ${revisionNumber}, ${request.actor.memberId}, 'restored', ${note}, ${changedAt}
    )
  `;
}
...
export function createPostgresArticleDraftRepository(
  database: PostgresDatabase,
  ...
  async function runPublication(
    ...
    const statements = [
      authoringAssertion(request.workspaceId, "postgres"),
      actorAssertion(request.workspaceId, request.actor, changedAt, specification.roles),
      categoryAssertion(request.workspaceId, category, true),
      workflowHeadAssertion(request, state, specification.allowedStates),
      ...(specification.independentActor ? [independentActorAssertion(request)] : []),
      ...(approvalEventId
        ? [reviewEventInsert(request, approvalEventId, "approved", note, changedAt)]
        : []),
      ...lockSlugStatements(request.workspaceId, request.articleId, state.slug),
      slugAssertion(request.workspaceId, request.articleId, state.slug, false),
      ...lockAssetStatements(request.workspaceId, assetHashes, undefined),
      revisionAssetSetAssertion(request.workspaceId, request.revisionId, assetHashes),
      ...
      sql`
        update articles set
          ...
          position = ${state.position},
          published_at = coalesce(published_at, ${changedAt}),
      ...
      sql`
        ...
        select ${request.articleId}, revision_asset.asset_id, ${request.workspaceId}, ${changedAt}
        from article_revision_assets revision_asset
      ...
      `,
      sql`
        delete from article_slug_claims claim
        where claim.workspace_id = ${request.workspaceId}
          and claim.article_id = ${request.articleId}
          and claim.normalized_slug = ${state.articleSlug}
          and claim.normalized_slug <> ${state.slug}
          and not exists (
            select 1 from articles
            where workspace_id = claim.workspace_id
              and id = claim.article_id
              and slug = claim.normalized_slug
          )
          and not exists (
            select 1 from article_heads
            where workspace_id = claim.workspace_id
              and article_id = claim.article_id
              and working_slug = claim.normalized_slug
          )
      `,
  ...
  async function runUnpublish(
    ...
    const statements = [
      ...
      sql`
        update articles set status = 'draft', updated_at = ${changedAt}
        where workspace_id = ${request.workspaceId}
          and id = ${request.articleId}
  ...
  async function runArchiveTransition(
    ...
    const statements: SQL[] = [
      ...
      archiveHeadAssertion(request, state, restoring),
      ...(!restoring && state.articleStatus === "published"
        ? [
            sql`
              update article_heads set
      ...
      sql`
        update article_heads set
          review_state = ${nextState},
          submitted_by_member_id = ${nextState === "in_review" ? state.submittedByMemberId : null},
          archived_at = ${restoring ? null : changedAt},
      ...
      ...(restoring
        ...
        : [
            sql`
              update article_preview_grants grant_record set
                revoked_at = greatest(grant_record.created_at, ${changedAt}),
                revoked_by_member_id = ${request.actor.memberId}
              where grant_record.workspace_id = ${request.workspaceId}
                and grant_record.revoked_at is null
                and exists (
                  select 1 from article_revisions revision
                  where revision.workspace_id = grant_record.workspace_id
                    and revision.id = grant_record.revision_id
                    and revision.article_id = ${request.articleId}
                )
            `,
          ]),
  ...
  async function runRestoreRevision(
    ...
    const statements = [
      ...
      ...lockSlugStatements(request.workspaceId, request.articleId, source.slug),
      slugAssertion(request.workspaceId, request.articleId, source.slug, false),
      ...lockAssetStatements(request.workspaceId, historicalAssets.hashes, undefined),
      assetAssertion(
        request.workspaceId,
        historicalAssets.hashes,
        undefined,
      ...
      sql`
        ...
        where claim.workspace_id = ${request.workspaceId}
          and claim.article_id = ${request.articleId}
          and claim.normalized_slug = ${state.workingSlug}
          and claim.normalized_slug <> ${source.slug}
          and not exists (
            select 1 from articles
            where workspace_id = claim.workspace_id
              and id = claim.article_id
      ...
      `,
      sql`
        update article_slug_claims claim set working_claim = false
        where claim.workspace_id = ${request.workspaceId}
          and claim.article_id = ${request.articleId}
          and claim.normalized_slug = ${state.workingSlug}
          and claim.normalized_slug <> ${source.slug}
  ...
  return withAuthoringErrorBoundary<ArticleDraftRepository>({
    ...
    async createDraftArticle(request) {
      ...
      const statements = [
        authoringAssertion(request.article.workspaceId, "postgres"),
        ...
        ...lockAssetStatements(request.article.workspaceId, hashes, manifestId),
        assetAssertion(
          request.article.workspaceId,
          hashes,
          manifestId,
        ...
        sql`
          insert into articles (
            ...
            ${request.article.slug}, ${request.article.title}, ${request.article.mdx},
            null, 'draft', ${request.article.isFaq}, ${request.article.authorName},
            ${request.article.position}, null, ${changedAt}, ${changedAt}
          )
        `,
        upsertSlugClaim(
          request.article.workspaceId,
    ...
    async saveDraftArticle(request) {
      ...
      const statements = [
        ...
        sql`
          delete from article_slug_claims claim
          where claim.workspace_id = ${request.article.workspaceId}
            and claim.article_id = ${request.article.id}
            and claim.normalized_slug = ${state.workingSlug}
            and claim.normalized_slug <> ${request.article.slug}
            and not exists (
              select 1 from articles
              where workspace_id = claim.workspace_id
                and id = claim.article_id
