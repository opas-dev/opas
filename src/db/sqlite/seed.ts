// ABOUTME: Installs one complete revision-aware demo profile in a SQLite or D1 transaction.
// ABOUTME: Uses one native D1 batch so every seed record commits or rolls back together.
...
import { articleEvidenceCommitStatements } from "@/db/sqlite/evidence-repository";
import {
  articleAssets,
  articleHeads,
  articleRevisions,
...
async function readSeedState(
  ...
  const executable = database as DrizzleD1Database<typeof schema>;
      ...
      value: sql<number>`(
        ...
        where article.workspace_id = ${workspaceId}
          and (
            ...
            or working_revision.slug <> head.working_slug
            or (
            ...
            or (
              ...
              and (
                ...
                or article.category_id <> published_revision.category_id
                or article.slug <> published_revision.slug
                or article.title <> published_revision.title
                or article.mdx <> published_revision.mdx
                or article.is_faq <> published_revision.is_faq
                or article.author_name <> published_revision.author_name
                or article.position <> published_revision.position
...
function seedPreconditionAssertion(plan: DemoSeedPlan) {
  return sql`
    ...
    ) then 1 else json_extract('DEMO_SEED_PRECONDITION_FAILED', '$') end
  `;
}

function initialSeedStatements(database: SqliteDatabase, plan: DemoSeedPlan) {
  const executable = database as DrizzleD1Database<typeof schema>;
  const statements: SQL[] = [
    seedPreconditionAssertion(plan),
    authoringAssertion(plan.workspaceId, "sqlite"),
    executable
      .insert(categories)
      .values(
  ...
  if (revisionAssets.length > 0) {
    statements.push(
      executable.insert(articleRevisionAssets).values(revisionAssets).getSQL(),
    );
  }
  statements.push(
    executable
  ...
  if (publicAssets.length > 0) {
    statements.push(executable.insert(articleAssets).values(publicAssets).getSQL());
  }
  ...
  if (events.length > 0) {
    statements.push(executable.insert(articleReviewEvents).values(events).getSQL());
  }
  return statements;
}
...
function injectedStatements(
  ...
  return [
    ...
    sql`select json_extract('DEMO_SEED_INJECTED_FAILURE', '$')`,
...
export async function reconcileSqliteDemoSeed(
  database: SqliteDatabase = getD1Database(),
...
export async function seedD1(
  ...
  await reconcileSqliteDemoSeed(database, options);
}
